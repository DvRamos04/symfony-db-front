...
class HomeController extends AbstractController {
  ...
  public function index(): Response {
    ...
    $stmt=$pdo->prepare("SELECT section,k,male,female,other,total FROM etl_summary WHERE execution_id=?");
    $stmt->execute([$selected]); $rows=$stmt->fetchAll(\PDO::FETCH_ASSOC);

    $gender=["male"=>0,"female"=>0,"other"=>0]; $ages=[]; $os=[];
    foreach($rows as $r){ if($r["section"]==="gender"){$gender[$r["k"]]=(int)$r["total"];}
                           if($r["section"]==="age"){$ages[$r["k"]]=(int)$r["total"];}
                           if($r["section"]==="os"){$os[$r["k"]]=(int)$r["total"];} }
    ksort($ages);

    return $this->render("home.html.twig",[
      "execs"=>$execs, "selected"=>$selected, "gender"=>$gender, "ages"=>$ages, "os"=>$os
    ]);
  }
}
